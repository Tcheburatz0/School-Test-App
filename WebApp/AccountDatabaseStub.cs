...
namespace WebApp
{
    class AccountDatabaseStub : IAccountDatabase
    {
        ...
        private bool _getAccountOnce = true;
        ...
        public List<Account> GetAccountOnce()
        {
            if (_getAccountOnce)
            {
                _getAccountOnce = false;

                var accounts = new List<Account>(_accounts.Count);
                foreach (var kvp in _accounts)
                {
                    accounts.Add(kvp.Value?.Clone());
                }
                return accounts;
            }
            return null;
